...
public class Database {
	...
	public static Account cursorToAccount(Cursor accounts) {
		...
		account.setId(accounts.getString(accounts
				.getColumnIndex(Database.ACCOUNTS_ID)));
		account.setHolder(accounts.getString(accounts
				.getColumnIndex(Database.ACCOUNTS_HOLDERS)));
		account.setBank(accounts.getString(accounts
	...
	public static void populateAccounts(Spinner spinnerAccounts) {
		...
		Cursor accounts = db.query(Database.ACCOUNTS_TABLE_NAME, null, null,
				null, null, null, null);
	...
	public static boolean updateAccountBalance(SQLiteDatabase db,
			String accountId, String transType, double amount, String transDate) {
		try {
			if (transType.equals("d"))
				db.execSQL(" update " + Database.ACCOUNTS_TABLE_NAME
						+ " set balance = balance + " + amount + " where "
						+ Database.ACCOUNTS_ID + " = " + accountId);
			else
				db.execSQL(" update " + Database.ACCOUNTS_TABLE_NAME
						+ " set balance = balance - " + amount + " where "
						+ Database.ACCOUNTS_ID + " = " + accountId);
	...
	public static boolean addTransaction(Context context, String accountId,
			String transType, String transDate, String transAmount,
			String chequeNo, String chequeParty, String chequeDetails,
			String remarks) {
		...
		try {
			...
			long rowid = db.insert(Database.TRANSACTIONS_TABLE_NAME, null,
					values);
			Log.d("Accounts", "Inserted into TRANSACTIONS " + rowid);
			if (rowid != -1) {
				boolean done = Database.updateAccountBalance(db, accountId,
						transType, Double.parseDouble(transAmount), transDate);
